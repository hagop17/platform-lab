...
- **Pre-commit hooks:** ruff (`--fix`), ruff-format, pyright, pytest (offline, `HF_HUB_OFFLINE=1`), `terraform fmt -check -recursive terraform`, check-yaml, trailing-whitespace, end-of-file-fixer — all must pass before committing.
- **Validate Terraform offline:** `terraform -chdir=terraform/<stack> init -backend=false && terraform -chdir=terraform/<stack> validate` for `bootstrap` and `eks`. `-backend=false` is what lets this run with no AWS credentials, so it works in the dev container and in CI. Applying either stack is host-only — see `docs/superpowers/plans/2026-08-14-eks-cluster.md` Tasks 12–14.
...
- **Build and push the app image to ECR (host, human-run):** `docker build --provenance=false -t "$ECR:$SHA" .` where `SHA` is the git short SHA and `ECR` comes from `terraform -chdir=terraform/bootstrap output -raw ecr_repository_url`. `--provenance=false` is required, not cosmetic — see the ECR retention gotcha below.
...
- **ECR's "keep 3 images" rule counts manifests, not pushes** (`terraform/bootstrap/ecr.tf`). BuildKit attaches a provenance attestation by default, and an attestation forces the result into an OCI image index — so one default `docker build` push lands as *three* manifests (index + image + attestation), silently turning the retention rule into "keep one." Worse, a rule can expire an untagged child manifest while the tagged index still points at it, leaving a broken tag that `IMMUTABLE` forbids re-pushing. Build with `--provenance=false` so one push is one manifest.
- Ruff `target-version = "py311"` even though `requires-python = ">=3.12"`.
- `.gitignore` covers `.terraform/`, `*.tfstate*`, `*.tfvars`, `backend.hcl` — all four are live, since both Terraform stacks keep the account ID and state out of git entirely. `data/` and `*.db` are still unused.
- **No account ID literal in any committed file.** It reaches Terraform only through the gitignored `terraform.tfvars` / `backend.hcl`, and reaches `k8s/app-deployment.yaml` only via a deploy-time `sed` — the committed manifest keeps its `ACCOUNT_ID` / `REPLACE_WITH_GIT_SHA` placeholders permanently. Don't "helpfully" substitute them in the file.
- `docs/` holds five long-form docs: `tpr_rag_spec.md` (RAG design + post-build findings), `devcontainer-spec.md` (sandboxed dev container), `guides/rag-embeddings-primer.md` (standalone embeddings primer, CC BY 4.0), and under `superpowers/specs/` the eCFR-sources and EKS-cluster design specs, each paired with a plan in `superpowers/plans/`. Specs and plans are **point-in-time records** — where execution contradicted them, a "Corrected during execution" note is added in place rather than the original being rewritten. Two earlier planning specs (`otel-demo-spec.md`, `otel-demo-llm-analysis-spec.md`) were deleted as stale/superseded — don't reference them.
